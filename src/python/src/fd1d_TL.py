...
def TL_Nconductors_losses(zSteps: int, tSteps: int, l: np.ndarray, c: np.ndarray, r: np.ndarray, g: np.ndarray):
    ...
    source1, source2 = getTerminalMatrices(c, g, rs, dZ, dT)
    load1, load2     = getTerminalMatrices(c, g, rl, dZ, dT)

    cg1, cg2 = getUpdateMatrices(c, g, dZ, dT)
    lr1, lr2 = getUpdateMatrices(l, r, dZ, dT)
